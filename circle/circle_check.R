...
points(t(t$truth[,idx_save[g_ran]+1]), col="red")
points(t(t$mles[,idx_save[g_ran]+1]), col="blue")
...
quantile(s[[1]]$tau2[g_ran, 5001:10000], c(.01, .5, .99))
plot(s[[1]]$beta[1,9001:10000,g_ran], s[[1]]$tau2[g_ran,9001:10000])
...
beta_df <- data.frame(x = as.numeric(s[[1]]$beta[1,5001:10000,]),
                      y = as.numeric(s[[1]]$beta[2,5001:10000,]))
...
ggplot(aes(x1, x2, weight=w)) + geom_hex(bins=25) + scale_fill_continuous() +
  geom_circle(data=true_means, inherit.aes = F, aes(x0=x0,y0=y0,r=r), color="red", size=1)
