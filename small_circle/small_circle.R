...
priors <- formatPriors(K = G, prior_mean = rep(0,2), prior_sd = rep(sigma0,2), alpha = 1, a = 3, b = 2, A = 10, B = 1)
...
out3 <- mcmc(data, priors, methodPi="stickBreaking", alpha_fixed = F, n_iter = n_iter,
             n_save_P=1, idx_save = idx_save, thin = 1, verbose =2, C=C)
out4 <- mcmc(data, priors, methodPi="symmDirichlet", alpha_fixed = F, n_iter = n_iter, s_RW_alpha=.5,
            n_save_P=1, idx_save = idx_save, thin = 1, verbose =0, C=C)
